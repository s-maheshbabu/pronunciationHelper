...
exports.handler = function (event, context) {
    try {
        ...
        if (event.session.application.applicationId !== "amzn1.echo-sdk-ams.app.22799827-aae1-4115-9b48-e2b74e33ee03") {
...
function getWelcomeResponse(callback) {
    ...
    var cardOutput = "Examples: \n" +
                     "Pronounce D. O. G.\n" +
                     "How to pronounce B. I. T. S.\n" +
                     "What is the pronunciation for C. A. T.\n" +
                     "Ask pnonunciations to pronounce P. I. L. A. N. I.\n";
    var speechOutput = "<speak> Welcome to Pronunciations. " +
            "You can say things like, pronounce B. I. T. S. </speak>";
    // If the user either does not reply to the welcome message or says something that is not
    // understood, they will be prompted again with this text.
    var repromptText = "<speak> What word do you want the pronunciation for? You can say things like, what is the pronunciation for P. I. L. A. N. I. </speak>";
...
function getHelpResponse(callback) {
    var speechOutput = "<speak> I can help you pronounce English words in American accent. You just need to spell the word you need the pronunciation for. " +
...
function pronounceTheWord(intent, session, callback) {
    ...
    if (wordToBePronoucnedSlot !== undefined && wordToBePronoucnedSlot.value !== undefined) {
        ...
        if(hasLowerCase(wordToBePronoucned)) {
            ...
            if(!sessionAttributes || !sessionAttributes.numberOfFailedAttempts || sessionAttributes.numberOfFailedAttempts < 3) {
                shouldEndSession = false;
                speechOutput = "<speak> I didn't get that. Please try again. </speak>";
                cardOutput = "I heard \"" + wordToBePronoucned + "\" but I do not know how to pronounce it. Please try again."; 
            }
            else {
                speechOutput = "<speak> Sorry, am having trouble understanding. Please try again later. Good bye. </speak>";
                cardOutput = "I heard \"" + wordToBePronoucned + "\" but I do not know how to pronounce it. Sorry."; 
            }
        }
        else if(hasWhiteSpaces(wordToBePronoucned)) {
            wordToBePronoucned = wordToBePronoucned.replace(/ /g, '');
            speechOutput = "<speak> I would pronounce it as " + wordToBePronoucned + ". </speak>";
            cardOutput = "Now that you know how to pronounce " + wordToBePronoucned + ", you can ask Alexa for its meaning by saying \"Alexa, define " + wordToBePronoucned + "\"";
            cardTitle = "Pronunciation of " + wordToBePronoucned;
        }
        else {
            speechOutput = "<speak> It is pronounced as " + wordToBePronoucned + ". </speak>";
            cardOutput = "Now that you know how to pronounce " + wordToBePronoucned + " you can ask Alexa for its meaning by saying \"Alexa, define " + wordToBePronoucned + "\"";
            cardTitle = "Pronunciation of " + wordToBePronoucned;
        }
    } else {
        ...
        if(!sessionAttributes || !sessionAttributes.numberOfFailedAttempts || sessionAttributes.numberOfFailedAttempts < 3) {
            shouldEndSession = false;
            speechOutput = "<speak> I didn't get that. Please try again. </speak>";
            cardOutput = "Am sorry, am having trouble understanding. Please try again.";   
        }
        else {
            speechOutput = "<speak> Sorry, am having trouble understanding. Please try again later. Good bye </speak>";
            cardOutput = "Am sorry, am having trouble understanding. Please try again."; 
